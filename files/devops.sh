...
echo "🔧 > Agregando variables de entorno para DevOps..."
    ...
    exec_until_done curl -sSL -o $DEVOPS_VARS $REMOTE_DEVOPS_VARS || { echo "Error descargando $REMOTE_DEVOPS_VARS"; exit 1; }
...
exec_until_done curl -sSL $TUNNEL_INSTALL_SCRIPT | bash

TUNNEL_CONFIG_SCRIPT=$REMOTE_REPO/files/tunnel-config.sh
echo "TUNNEL_CONFIG_SCRIPT: $TUNNEL_CONFIG_SCRIPT"
exec_until_done curl -sSL $TUNNEL_CONFIG_SCRIPT | bash